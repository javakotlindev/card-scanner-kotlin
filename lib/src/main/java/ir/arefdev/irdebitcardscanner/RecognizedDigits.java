...
class RecognizedDigits {
    ...
    private final ArrayList<Float> confidence;
    ...
    static RecognizedDigits from(RecognizedDigitsModel model, Bitmap image, CGRect box) {
        final Bitmap frame = Bitmap.createBitmap(image, Math.round(box.getX()), Math.round(box.getY()),
        ...
        for (int col = 0; col < kNumPredictions; col++) {
            ...
            if (argAndConf.getConfidence() < kDigitMinConfidence) {
                digits.add(kBackgroundClass);
            } else {
                digits.add(argAndConf.getArgMax());
            }
            confidence.add(argAndConf.getConfidence());
        }
