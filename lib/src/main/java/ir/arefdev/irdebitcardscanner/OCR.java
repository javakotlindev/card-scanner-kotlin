...
class OCR {
    ...
    private ArrayList<DetectedBox> detectBoxes(Bitmap image) {
        ...
        for (int row = 0; row < findFour.getRows(); row++) {
            for (int col = 0; col < findFour.getCols(); col++) {
                if (findFour.hasDigits(row, col)) {
                    ...
                    DetectedBox box = new DetectedBox(row, col, confidence, findFour.getRows(),
                            findFour.getCols(), findFour.getBoxSize(), findFour.getCardSize(), imageSize);
                    boxes.add(box);
                }
            }
        }
    ...
    private String runModel(Bitmap image) {
        ...
        RecognizeNumbers recognizeNumbers = new RecognizeNumbers(image, findFour.getRows(), findFour.getCols());
        ...
        if (expiryBoxes.size() > 0) {
            ...
            this.expiry = Expiry.Companion.from(recognizedDigitsModel, image, expiryBox.getRect());
            if (this.expiry != null) {
